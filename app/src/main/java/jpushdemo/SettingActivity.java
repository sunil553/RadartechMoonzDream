...
public class SettingActivity extends InstrumentedActivity implements OnClickListener {
   ...
   @Override
	...
	/**
	 *Set the allowable notification time
	 */
	private void setPushTime(){
		int startime = startTime.getCurrentHour();
		int endtime = endTime.getCurrentHour();
		if (startime > endtime) {
			Toast.makeText(SettingActivity.this, "The start time can not be greater than the end time", Toast.LENGTH_SHORT).show();
			return;
		}
		...
		//Call the JPush api to set the Push time
